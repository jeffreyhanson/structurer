...
/*======================================================= 
  ...
  =========================================================*/
#define VERSION "2.3.1 (Febrauary 2009)"
...
void GetNumLocations (struct IND *ind) {
  ...
  for (i=0; i<NUMINDS; i++) {
    ...
    if (ind[i].Location > maxloc) maxloc = ind[i].Location;
  }
  freq = malloc((maxloc+1)*sizeof(int));
  for (i=0; i<=maxloc; i++) freq[i]=0;
  for (i=0; i<NUMINDS; i++)
    freq[ind[i].Location]++;
  pos=0;
  for (i=0; i<=maxloc; i++) {
  ...
  parameters to appropriate values */
{
  ...
  *sumsqlikes = 0.0;
  if (INTERMEDSAVE > 0)
    ...
    *savefreq = 0;
  if (LINKAGE) InitializeR (R, sumR, varR);
  if (RECESSIVEALLELES)
    {
  ...
  else CountAlleles (Geno, NumAlleles, Translation, Recessive);  /*recode alleles to {0,..,1-k} */
  if (STARTATPOPINFO)
    InitFromGeogPop (Geno, Individual, Z, 1);
  /*set Z to geog origin */
  else
...
void UpdateAlpha (double *Q, double *Alpha, struct IND *Individual, int rep)
{
  ...
  if (!((NOADMIX) && ((rep >= ADMBURNIN) || (rep > BURNIN)))) {
    ...
    if (POPALPHAS) numalphas = MAXPOPS; 
    else numalphas = 1;
	...
	if (POPALPHAS)  { /*different alphas in each population*/
	  ...
	  for (i=0; i<MAXPOPS; i++) sumalphas += Alpha[i];
...
double LogProbQ_LocPrior_loc(double *Q, double *Alpha, struct IND *Individual, int loc) {
  ...
  for (ind=0; ind<NUMINDS; ind++) {
    if (Individual[ind].myloc!=loc) continue;
    for (pop=0; pop<MAXPOPS; pop++) {
...
void UpdateAlphaLocPrior(double *Q, double *Alpha, double *LocPrior,
  ...
  for (pop=0; pop < MAXPOPS; pop++) {
    ...
    if (newalpha >= ALPHAMAX || newalpha <= 0.0) continue;
    diff = 0.0;
    for (loc=0; loc<NUMLOCATIONS; loc++)
  ...
  the population frequencies, P.  Uniform prior for lambda assumed */
{
  ...
  if ((new > 0.0) && (new < LAMBDAMAX))
    {    
      if (FREQSCORR) stoppop=1; 
      else stoppop = MAXPOPS;
	...
	if (NumAlleles[loc] > 1)
	  {
	    ...
	    for (pop=0; pop<stoppop; pop++)
	      for (allele=0; allele<NumAlleles[loc]; allele++)
		{
		  if (FREQSCORR) sumlogp += log(Epsilon[EpsPos(loc,allele)]);
		  else sumlogp += LogP[PPos(loc,pop,allele)];
		}
...
FlikeFreqs (double f, double *Epsilon, double *LogP, int *NumAlleles, int pop)
{
  ...
  for (loc=0; loc<NUMLOCI; loc++)
	...
	{	
      ...
      if (NumAlleles[loc]==0) sum -=mylgamma(frac); //should not be counting sites with all missing data
    }
  return sum;
...
/*update the correlation factor, Fst, for each population*/
{
  ...
  if (ONEFST) numpops1 = 1; else numpops1 = MAXPOPS; 
  ...
  for (pop1 = 0; pop1 < numpops1; pop1++)
      ...
      if (newf > 0.0 && newf<1.0)	
	{
	  ...
	  if (ONEFST) numpops2 = MAXPOPS; else numpops2 = pop1+1;
	  ...
	  if (logprobdiff >= 0.0)	/*accept new f */
	    ...
	    {			/*accept new parameter with prob p */
	      threshold = exp (logprobdiff);
	      unifrv = rnd ();
	      if (unifrv < threshold)
		for (pop2 = pop1; pop2 < numpops2; pop2++)
...

{
  ...
  for (ind = 0; ind < NUMINDS; ind++)
	...
	if (LOCPRIOR) PriorQ1 = &Alpha[AlphaPos(Individual[ind].myloc, 0)];
	...
	for (pop = 0; pop < MAXPOPS; pop++)	/*make a vector of log probs for each pop */
	  {
	    ...
	    if (pop==0 || ProbsVector[pop] > max) max = ProbsVector[pop];
	  }
...
void UpdateLocPriorAdmix(double *Q, double *LocPrior, double *Alpha, struct IND *Individual) {
  ...
  if (newPP > 0.0 && newPP < MAXLOCPRIOR) {
	...
	diff += globalpha*PPdiff*log(localpha) - PPdiff*localpha - mylgamma(globalpha*newPP) + mylgamma(globalpha*currPP) + globalpha*newPP*log(newPP) - globalpha*currPP*log(currPP);
      }
    }
    if (diff > 0.0 || RandomReal(0, 1) < exp(diff))
      LocPrior[0] = newPP;
...
void UpdateLocPrior(double *Q, double *LocPrior, double *Alpha, struct IND *Individual) {
  if (NOADMIX) UpdateLocPriorNoAdmix(Q, LocPrior, Individual);
  else UpdateLocPriorAdmix(Q, LocPrior, Alpha, Individual);
}
...
//ambiguous due to recessive markers or inbreeding.
{
  ...
  int ind, loc, allele, line, dom, toggle;
		  ...
		  && PreGeno[GenPos (ind, 1, loc)] != MISSING)
		{
		  if (PreGeno[GenPos (ind, 0, loc)] ==
							...
							
			{
			  AlleleProbs[0] =
			    P[PPos
			      (loc, Z[ZPos (ind, 0, loc)],
			       Recessive[loc])] * P[PPos (loc, Z[ZPos(ind,1,loc)], 
	 ...
	 double *RTransitProb, double *Mapdistance, double *Phase,int *Phasemodel)
{
  ...
  if (PHASED)
	...
	for (loc = 1; loc < NUMLOCI; loc++)
	  {
	    if (Mapdistance[loc] < 0) problinked=0;  /*this is the code for unlinked loci*/
	    else problinked = exp (-Rec * Mapdistance[loc]);
	    ...
	    for (pop = 0; pop < MAXPOPS; pop++)
	      if (Geno[GenPos (ind, line, loc)] == MISSING)
		RTransitProb[RTransitProbPos (loc, line, pop)] = (1.0-problinked)*IndividualQ[pop]*asum + problinked* RTransitProb[RTransitProbPos (loc - 1, line, pop)];
	    /*necessary to incorporate recombination even if the data is missing */
	      else
		RTransitProb[RTransitProbPos (loc, line, pop)] = ((1.0-problinked)*IndividualQ[pop]*asum+problinked * RTransitProb[RTransitProbPos (loc - 1, line, pop)]) * P[PPos (loc, pop, Geno[GenPos (ind, line, loc)])];
	...
	for (pop2 = 0; pop2 < MAXPOPS; pop2++)
	  {
	    ...
	    if (Phasemodel[ind]==1)
	      RTransitProb[DiploidRTransitProbPos (0, pop, pop2)] = IndividualQ[pop] * IndividualQ[pop2] * (Phase[PhasePos (ind, 0)] * tempP00 * tempP11 + (1.0 - Phase[PhasePos (ind, 0)]) * tempP01 * tempP10);
	    else
      ...
      if (Phasemodel[ind]==1)
	{
	  ...
	  for (loc = NUMLOCI - 1; loc > -1; loc = loc - 1)
		...
		for (pop2 = 0; pop2 < MAXPOPS; pop2++)
	      ...
	      if (loc < NUMLOCI - 1)
		{
		  if (Mapdistance[loc+1] < 0) problinked=0;  /*this is the code for unlinked loci*/
		  else problinked = exp (-Rec * Mapdistance[loc + 1]);
		  for (pop = 0; pop < MAXPOPS; pop++)
		    for (pop2 = 0; pop2 < MAXPOPS; pop2++)
		      {
...
/* updates Z and R, assuming that the data is phased */
{
  ...
  for (ind = 0; ind < NUMINDS; ind++)
	...
	IndividualQ[pop] = Q[QPos (ind, pop)];
      indlike = Forward (Z, IndividualQ, P, Geno, 
			 R[ind], ind, RTransitProb, Mapdistance, Phase,Phasemodel);
      currentloglikelihood += indlike;
      if (sumindlikes!=NULL) 
	sumindlikes[ind] += exp(indlike-indlike_norm[ind]);
		
      Backward (Z, SiteBySiteSum, IndividualQ, R[ind], ind, 
		Mapdistance, RTransitProb, rep, Z1, Phase, P, Geno,Phasemodel);
...
/* updates Z and R, assuming that the data is phased */
{
  ...
  for (ind = 0; ind < NUMINDS; ind++)
	...
	IndividualQ[pop] = Q[QPos (ind, pop)];
      ...
      currentloglikelihood = Forward (Z, IndividualQ, P, Geno, R[ind], ind, RTransitProb, Mapdistance, Phase,Phasemodel);
      Backward (Z, SiteBySiteSum, IndividualQ, R[ind], ind, Mapdistance, RTransitProb, rep, Z1, Phase, P, Geno,Phasemodel);
...
int main (int argc, char *argv[])
{
  ...
  if (RECESSIVEALLELES)
    {
      ...
      if (PreGeno == NULL || Recessive == NULL){printf ("Error (3) in assigning memory\n");Kill ();}
    }
  ...
  else ReadInputFile (Geno, Mapdistance, Markername, Individual, Phase, Recessive);
	
  if (RECESSIVEALLELES) MAXALLELES = FindMaxAlleles (PreGeno, Recessive);
  else MAXALLELES = FindMaxAlleles (Geno, Recessive);
  ...
  if ((Translation == NULL) || (NumAlleles == NULL) || (Z == NULL) || (Z1 == NULL) || (Q == NULL) ||
      (P == NULL) || (LogP==NULL) || (R == NULL) || (sumR == NULL) || (varR == NULL) || (Epsilon == NULL) || (Fst == NULL) || (NumLociPop == NULL) ||
